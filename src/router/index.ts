...
router.get("/api/kv/:key", (req: Request & { params: Record<string, string | undefined> }, env: Env) => kvGetHandler(req, env, req.params.key ?? ""));
router.get("/api/video", (_req: Request, env: Env) => videoListHandler(env));
router.get("/api/video/:filename", (req: Request & { params: Record<string, string | undefined> }, env: Env) => videoHandler(req, env, req.params.filename ?? ""));
router.get("/api/live/:path", (req: Request & { params: Record<string, string | undefined> }, env: Env) => liveStreamHandler(req, env, req.params.path ?? ""));
