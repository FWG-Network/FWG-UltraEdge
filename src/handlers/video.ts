...
export async function videoHandler(req: Request, env: Env, filename: string): Promise<Response> {
  if (!filename) return Response.json({ error: "Bad Request", message: "Filename required" }, { status: 400 });
...
export async function liveStreamHandler(req: Request, env: Env, path: string): Promise<Response> {
  if (!env.VIDEO_ORIGIN) return Response.json({ error: "Not Configured", message: "VIDEO_ORIGIN not set" }, { status: 503 });
