...
// Ultra Video Streaming: Range Requests + Adaptive + Live Proxy
// Fixed: Direct R2 body — no TransformStream infinite loop
// ══════════════════════════════════════════════════════════════════════
...
export async function streamR2Video(
  ...
  // ── Ranged request (seek / resume support) ────────────────────────
  if (rangeHeader) {
  ...
  // ── Full stream — direct R2 body (no TransformStream) ─────────────
  const obj = await env.R2.get(filename);
  if (!obj) return Response.json({ error: "Video not found" }, { status: 404 });
