...
export const CACHE_TTL = {
  HEALTH: 10, API: 60, VIDEO: 31_536_000, STATIC: 86_400, DEFAULT: 30,
} as const;
...
export function buildCacheKey(req: Request): Request {
  ...
  ["utm_source","utm_medium","utm_campaign","fbclid","gclid","_ga","ref"]
...
export function resolveCachePolicy(pathname: string): { ttl: number; tag: CacheTag; immutable: boolean } {
...
export function applyCacheHeaders(res: Response, ttl: number, immutable: boolean, tag: CacheTag, hit: "HIT"|"MISS"|"STALE"): Response {
...
export async function withEdgeCache(
  ...
  if (kvRaw) {
    ...
    ctx.waitUntil(next(req).then(fresh =>
