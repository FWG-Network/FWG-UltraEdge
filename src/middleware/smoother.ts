...
export async function withCoalescing(
  ...
  if (IN_FLIGHT.has(key)) {
    const s = await IN_FLIGHT.get(key)!;
    return s.clone();
  }
...
const getC = (s: string): CircuitState =>
  CIRCUIT.get(s) ?? { failures: 0, lastFailure: 0, state: "CLOSED" };

export function recordSuccess(s: string): void {
  CIRCUIT.set(s, { failures: 0, lastFailure: 0, state: "CLOSED" });
}
export function recordFailure(s: string): void {
  const c = getC(s);
  const f = c.failures + 1;
  CIRCUIT.set(s, {
    failures: f,
    lastFailure: Date.now(),
    state: f >= CB.threshold ? "OPEN" : c.state,
  });
}
export function isOpen(s: string): boolean {
  const c = getC(s);
  if (c.state === "CLOSED") return false;
  if (c.state === "OPEN" && Date.now() - c.lastFailure > CB.timeout) {
    CIRCUIT.set(s, { ...c, state: "HALF_OPEN" });
    return false;
  }
  return c.state === "OPEN";
}
...
export function backoff(i: number, base = 100, cap = 3_000): number {
  const e = Math.min(cap, base * 2 ** i);
  return Math.floor(e + Math.random() * e * 0.3);
}
...
export function degradedResponse(pathname: string, env: Env): Response {
  ...
  return Response.json(
    fb[pathname] ?? { error: "Service Unavailable", message: "FWG-UltraEdge 🌍⚡", retry: true },
    {
      status: 503,
      headers: { "Retry-After": "10", "X-Degraded": "1", "Cache-Control": "no-store" },
    }
  );
}
